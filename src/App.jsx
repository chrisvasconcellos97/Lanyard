...
const INIT_PARTNERS = [
  { id:"p1", name:"LKQ", revenue:"$222M", tier:"Major", status:"red",
    ...
    attendees:[{ id:"a1", name:"Mark Scafati", title:"VP Sales and Marketing", poc:true, notes:"Primary decision maker" },
  ...
  { id:"p3", name:"KSI", revenue:"$20M", tier:"Mid", status:"yellow",
    ...
    openItems:[{ id:"oi2", text:"Finalize KSI invoice feed.", done:false },
               { id:"oi3", text:"Shop audit in progress.", done:false },
               { id:"oi4", text:"Discuss missing CAPA certifications.", done:false }],
    attendees:[{ id:"a5", name:"Eric Taylor",    title:"Sr. MSO Manager", poc:true,  notes:"Operational lead" },
  ...
  { id:"p4", name:"All Star Auto Parts", revenue:"$18.2M", tier:"Mid", status:"green",
    ...
    openItems:[{ id:"oi5", text:"Salvage pilot with Classic Collision - awaiting go-live.", done:false },
  ...
  { id:"p5", name:"Parts Authority", revenue:"$4.9M", tier:"Mid", status:"green",
    objective:"Support recent regional rollouts.", scheduledMeeting:"5/20 9:00 AM",
    unscheduled:false, rating:0, pastNotes:"",
  ...
  { id:"p6", name:"UCC", revenue:"$3.8M", tier:"Mid", status:"green",
    objective:"Check in and relationship building.", scheduledMeeting:"5/19 11:00 AM",
    unscheduled:false, rating:0, pastNotes:"",
    openItems:[],
    attendees:[{ id:"a15", name:"Joseph Tsai",  title:"SVP",      poc:true,  notes:"Senior sponsor" },
  ...
  { id:"p7", name:"Collision Auto Parts", revenue:"$2.9M", tier:"Growth", status:"green",
    objective:"Relationship check-in.", scheduledMeeting:"5/19 12:00 PM",
    unscheduled:false, rating:0, pastNotes:"",
    openItems:[],
  ...
  { id:"p9", name:"1-800 Radiator", revenue:"$1.9M", tier:"Growth", status:"red",
    ...
    openItems:[{ id:"oi8", text:"1,600 new integrations completed 5/13.", done:false },
               { id:"oi9", text:"3,000 shops still pending warehouse correction.", done:false }],
    attendees:[{ id:"a19", name:"Lindsay Klimek", title:"Sr. Manager Business Systems", poc:true,  notes:"Day to day lead" },
...
const INIT_SESSIONS = [
  ...
  { id:9,  day:"tue", time:"3:00 PM",  end:"5:00 PM",  title:"Business Session - Keynotes",location:"Grand Ballroom",          track:"Conference",      attendees:["C","K","T"], notes:"", status:"upcoming", partnerId:null, isParent:true,  isChild:false },
  ...
  { id:11, day:"tue", time:"3:30 PM",  end:"4:30 PM",  title:"Greg Horn - Collision Industry Update", location:"Grand Ballroom",track:"Keynote",        attendees:["C","K","T"], notes:"", status:"upcoming", partnerId:null, isParent:false, isChild:true  },
  { id:12, day:"tue", time:"4:30 PM",  end:"5:00 PM",  title:"Justin Rzepka - Legislative Update", location:"Grand Ballroom", track:"Keynote",          attendees:["C","K","T"], notes:"", status:"upcoming", partnerId:null, isParent:false, isChild:true  },
  ...
  { id:19, day:"wed", time:"11:30 AM", end:"12:00 PM", title:"Introduction to Board Candidates", location:"Grand Ballroom",    track:"Conference",      attendees:["C","K","T"], notes:"Joseph Tsai introduction", status:"upcoming", partnerId:null, isParent:false, isChild:false },
  ...
  { id:25, day:"wed", time:"7:00 PM",  end:"10:00 PM", title:"Offsite Dinner",             location:"TBD",                       track:"Meal/Reception",  attendees:["C","K","T"], notes:"Shots in the Night", status:"upcoming", partnerId:null, isParent:false, isChild:false },
...
function parseTime(t) {
  if (!t) return 0;
  var parts = t.split(" "); var period = parts[1]; var hm = parts[0].split(":");
  var h = parseInt(hm[0]); var m = parseInt(hm[1]) || 0;
...
function getUserId() {
  try {
    ...
    if (!id) { id = "u_" + Math.random().toString(36).slice(2,10); localStorage.setItem("lanyard_uid", id); }
    return id;
  } catch(e) { return "u_default"; }
}
function autoGenerateOpenSlots(sessions) {
  // Conference hours: 8am-10pm each day
  var CONF_START = 8 * 60;
  var CONF_END = 22 * 60;
  var MIN_GAP = 25; // minimum gap in minutes to create a slot
  var result = sessions.filter(function(s) { return s.track !== "Open Slot" || s.id < 100; });
  var autoIds = [];
  DAYS.forEach(function(d) {
    if (d.key === "thu") return;
    var daySessions = result.filter(function(s) {
    ...
    for (var i = 0; i < daySessions.length - 1; i++) {
      ...
      if (gap >= MIN_GAP) {
        var endH = Math.floor(nextStart / 60);
        var endM = nextStart % 60;
        var startH = Math.floor(currEnd / 60);
        var startM = currEnd % 60;
        var fmt = function(h, m) {
          var period = h >= 12 ? "PM" : "AM";
          var h12 = h > 12 ? h - 12 : h === 0 ? 12 : h;
          return h12 + ":" + (m < 10 ? "0" + m : m) + " " + period;
        };
        var slotLabel = gap >= 90 ? "Extended open window" : gap >= 60 ? "Open hour" : "Open slot";
        autoIds.push({
          id: "auto_" + d.key + "_" + i,
          day: d.key,
  ...
  // Remove old auto-generated slots and add new ones
  var manual = result.filter(function(s) { return !(typeof s.id === "string" && s.id.indexOf("auto_") === 0); });
  return manual.concat(autoIds);
}

function loadLocal() {
  try { var r = localStorage.getItem("lanyard_v"+DATA_VERSION); if (r) return JSON.parse(r); } catch(e) {}
  return null;
}
...
// Shared team ID - all ABPA 2026 users share the same partner/session data
var TEAM_ID = "abpa2026_team";

function sbLoad(cb) {
  var uid = getUserId();
  Promise.all([
    // Sessions and partners are SHARED - loaded by team ID
    ...
    // Personal prefs are per-user
    fetch(SUPABASE_URL+"/rest/v1/user_prefs?user_id=eq."+uid+"&select=hotel,quick_note", { headers:sbH() }),
    // Personal session notes are per-user
    fetch(SUPABASE_URL+"/rest/v1/user_prefs?user_id=eq."+uid+"_notes&select=hotel,quick_note", { headers:sbH() }),
  ]).then(function(rs) {
    return Promise.all([rs[0].json(), rs[1].json(), rs[2].json(), rs[3].json()]);
  }).then(function(data) {
    var sess = data[0], parts = data[1], prefs = data[2], notesRaw = data[3];
    // Merge personal notes back into sessions
    var personalNotes = {};
    try {
      if (notesRaw[0] && notesRaw[0].hotel) {
        personalNotes = notesRaw[0].hotel;
      }
    } catch(e) {}
    var mergedSessions = Array.isArray(sess) && sess.length > 0 ? sess.map(function(r) {
      var s = r.data;
...
function sbSave(sessions, partners, hotel, quickNote) {
  var uid = getUserId();
  // Save sessions and partners as SHARED (team ID)
  fetch(SUPABASE_URL+"/rest/v1/sessions?user_id=eq."+TEAM_ID, { method:"DELETE", headers:sbH() })
    .then(function() {
      if (sessions.length > 0) {
        // Strip personal notes before saving shared sessions
        var sharedSessions = sessions.map(function(s) {
          var shared = Object.assign({}, s);
          delete shared.takeaways; delete shared.actionItems;
          delete shared.commitmentsMade; delete shared.followUpDate; delete shared.rating;
          return { id:String(s.id)+"_shared", user_id:TEAM_ID, data:shared };
        });
        fetch(SUPABASE_URL+"/rest/v1/sessions", { method:"POST", headers:Object.assign({}, sbH(), { Prefer:"resolution=merge-duplicates" }), body:JSON.stringify(sharedSessions) });
      }
    }).catch(function(){});
  ...
  sessions.forEach(function(s) {
    var notes = {};
    if (s.takeaways) notes.takeaways = s.takeaways;
    if (s.actionItems) notes.actionItems = s.actionItems;
    if (s.commitmentsMade) notes.commitmentsMade = s.commitmentsMade;
    if (s.followUpDate) notes.followUpDate = s.followUpDate;
    if (s.rating) notes.rating = s.rating;
    if (Object.keys(notes).length > 0) personalNotes[String(s.id)] = notes;
  });
  ...
  // Save partners as SHARED
  fetch(SUPABASE_URL+"/rest/v1/partners?user_id=eq."+TEAM_ID, { method:"DELETE", headers:sbH() })
    .then(function() {
      if (partners.length > 0) {
        var rows = partners.map(function(p) { return { id:String(p.id)+"_shared", user_id:TEAM_ID, data:p }; });
        fetch(SUPABASE_URL+"/rest/v1/partners", { method:"POST", headers:Object.assign({}, sbH(), { Prefer:"resolution=merge-duplicates" }), body:JSON.stringify(rows) });
      }
    }).catch(function(){});
  // Save personal prefs
  fetch(SUPABASE_URL+"/rest/v1/user_prefs", {
...
function getRoleCtx(role, customTitle) {
  ...
  if (role === "dir") return "You are Pip, an AI assistant. The user is a Director with monthly partner touchpoints. Give strategic context. Be concise but substantive.";
...
function askPip(prompt, role, customTitle, ctx, cb) {
  ...
  fetch("https://api.anthropic.com/v1/messages", {
    ...
    body:JSON.stringify({ model:CLAUDE_MODEL, max_tokens:800, system:sys, messages:[{ role:"user", content:(ctx ? ctx+"\n\n" : "")+prompt }] }),
  }).then(function(r) { return r.json(); })
  .then(function(d) { cb(null, (d.content && d.content[0] && d.content[0].text) || "Pip couldn't respond right now."); })
  .catch(function(e) { cb(e, "Pip couldn't respond right now."); });
}
...
function Pill(props) {
  ...
  return (
    <span style={{ background:"rgba("+toRgb(color)+",0.15)", color:color, fontSize:10, fontWeight:600, padding:"3px 9px", borderRadius:20, border:"1px solid rgba("+toRgb(color)+",0.2)", whiteSpace:"nowrap" }}>
      {props.children}
    </span>
  );
}
...
function FL(props) {
  return <div style={{ fontSize:10, color:C.textMuted, marginBottom:4, textTransform:"uppercase", letterSpacing:"0.08em" }}>{props.children}</div>;
}
...
function Card(props) {
  ...
  return (
    <div onClick={props.onClick} style={Object.assign({ background:C.bgCard, border:"1px solid "+C.border, borderLeft:accent?"3px solid "+accent:"1px solid "+C.border, borderRadius:12 }, style)}>
      {props.children}
    </div>
  );
}
...
function GreenBtn(props) {
  var style = props.style || {};
  return (
    <button onClick={props.onClick} disabled={props.disabled} style={Object.assign({}, btnBase, { background:C.accent, color:"#fff", opacity:props.disabled?0.6:1 }, style)}>
      {props.children}
    </button>
  );
}
...
function SecBtn(props) {
  var style = props.style || {};
  return (
    <button onClick={props.onClick} style={Object.assign({}, btnBase, { background:C.bgCardAlt, color:C.textSub, border:"1px solid "+C.border }, style)}>
      {props.children}
    </button>
  );
}
...
function Toast(props) {
  ...
  return (
    <div style={{ position:"fixed", top:20, left:"50%", transform:"translateX(-50%)", zIndex:9999, background:C.accent, color:"#fff", padding:"11px 20px", borderRadius:24, fontWeight:600, fontSize:13, fontFamily:"'DM Sans',sans-serif", whiteSpace:"nowrap", boxShadow:"0 4px 20px rgba(74,155,130,0.4)" }}>
      {"✓ "+props.message}
    </div>
  );
}
...
function Modal(props) {
  ...
  return (
    <div onClick={handleBg} style={{ position:"fixed", top:0, left:0, right:0, bottom:0, background:"rgba(0,0,0,0.75)", zIndex:1000, display:"flex", alignItems:"center", justifyContent:"center", padding:16 }}>
      <div style={{ background:C.bgCard, border:"1px solid "+C.border, borderRadius:16, padding:22, width:"100%", maxWidth:wide?620:460, maxHeight:"92vh", overflowY:"auto", boxShadow:"0 24px 64px rgba(0,0,0,0.7)" }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:18 }}>
          <div style={{ fontSize:15, fontWeight:600, color:C.text }}>{props.title}</div>
          <button onClick={props.onClose} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:20, lineHeight:1 }}>x</button>
        </div>
        {props.children}
      </div>
    </div>
...
function EmptyState(props) {
  return (
    <div style={{ textAlign:"center", padding:"48px 24px" }}>
      ...
      {props.action && <GreenBtn onClick={props.onAction} style={{ fontSize:12, padding:"8px 20px" }}>{props.action}</GreenBtn>}
    </div>
  );
}
...
function AddressSearch(props) {
  ...
  useEffect(function() {
    ...
    timer.current = setTimeout(function() {
      ...
      setSugs(KNOWN_VENUES.filter(function(k) { return k.name.toLowerCase().includes(q) || k.address.toLowerCase().includes(q); }).slice(0,4));
    }, 200);
    return function() { clearTimeout(timer.current); };
  }, [query]);
  return (
    <div style={{ position:"relative" }}>
      <input value={query} onChange={function(e) { setQuery(e.target.value); onChange(e.target.value); }} placeholder={placeholder} style={inp}/>
      {sugs.length > 0 && (
        <div style={{ position:"absolute", top:"100%", left:0, right:0, background:C.bgDark, border:"1px solid "+C.border, borderRadius:8, zIndex:100, marginTop:2 }}>
          {sugs.map(function(s, i) {
            return (
              <div key={i} onClick={function() { setQuery(s.name); onChange(s.name); setSugs([]); }} style={{ padding:"10px 12px", borderBottom:"1px solid "+C.border, cursor:"pointer", fontSize:12 }}>
  ...
  return (
    <div style={{ background:"rgba("+toRgb(C.accent)+",0.06)", border:"1px solid rgba("+toRgb(C.accent)+",0.2)", borderRadius:10, padding:"12px 14px" }}>
      <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:done?10:0 }}>
        ...
        <div style={{ fontSize:11, fontWeight:600, color:C.accent, textTransform:"uppercase", letterSpacing:"0.07em", flex:1 }}>{props.label}</div>
        {!done && <button onClick={generate} disabled={loading} style={Object.assign({}, btnBase, { background:loading?"transparent":C.accent, color:loading?C.textMuted:"#fff", fontSize:10, padding:"5px 12px", opacity:loading?0.6:1 })}>{loading?"Thinking...":"Ask Pip"}</button>}
        {done && <button onClick={function() { setText(""); setDone(false); }} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:11, fontFamily:"'DM Sans',sans-serif" }}>refresh</button>}
      </div>
      ...
      {items.map(function(oi) {
        return (
          <div key={oi.id} style={{ display:"flex", alignItems:"flex-start", gap:8 }}>
            <input type="checkbox" checked={!!oi.done} onChange={function() { onToggle && onToggle(oi.id); }} style={{ marginTop:3, cursor:"pointer", flexShrink:0 }}/>
      ...
      {sorted.map(function(a) {
        return (
          <div key={a.id} style={{ background:C.bgDark, border:"1px solid "+C.border, borderRadius:8, padding:"8px 12px", display:"flex", alignItems:"center", gap:9 }}>
            <div style={{ width:28, height:28, borderRadius:"50%", background:"rgba("+toRgb(C.accent)+","+(a.poc?"0.2":"0.08")+")", display:"flex", alignItems:"center", justifyContent:"center", fontSize:11, fontWeight:600, color:a.poc?C.accent:C.textMuted, flexShrink:0 }}>{(a.name||"?").charAt(0)}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontSize:12, fontWeight:500, color:C.text }}>{a.name}{a.poc && <span style={{ color:C.yellow, fontSize:10, marginLeft:5 }}>POC</span>}</div>
    ...
    <Modal onClose={props.onClose} title={session.title} wide>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ display:"flex", gap:7, flexWrap:"wrap" }}>
          ...
          {props.hasConflict ? <Pill color={C.red}>Conflict</Pill> : null}
        </div>
        <div style={{ display:"flex", gap:14, flexWrap:"wrap" }}>
          {session.time ? <div style={{ fontSize:12, color:C.textSub }}>{"Time: "+session.time+(session.end?" - "+session.end:"")}</div> : null}
          {session.location ? (
            <div onClick={function() { if (vKey) setVM(session.location); }} style={{ fontSize:12, color:vKey?C.accent:C.textSub, cursor:vKey?"pointer":"default" }}>
          ...
          <div style={{ background:C.bgDark, border:"1px solid "+C.border, borderRadius:10, padding:"13px 15px" }}>
            ...
            {partner.openItems && partner.openItems.length > 0 ? (
              <div style={{ marginBottom:10 }}>
                <FL>Open Items</FL>
                <OpenItemsList items={partner.openItems} onToggle={function(id) { props.onUpdateP(partner.id, "openItems", partner.openItems.map(function(x){return x.id===id?Object.assign({},x,{done:!x.done}):x;})); }}/>
              </div>
            ) : null}
        ...
        <div style={{ borderTop:"1px solid "+C.border, paddingTop:12 }}>
          ...
          {[{f:"takeaways",l:"Takeaways",p:"Key things you learned..."},{f:"actionItems",l:"Action Items",p:"Who does what by when..."},{f:"commitmentsMade",l:"Commitments Made",p:"What did you promise them..."},{f:"followUpDate",l:"Follow Up Date",p:"When are you reconnecting..."}].map(function(x) {
            return (
              <div key={x.f} style={{ marginBottom:10 }}>
                <FL>{x.l}</FL>
                <textarea value={session[x.f]||""} onChange={function(e) { upd(x.f,e.target.value); }} placeholder={x.p} style={ta}/>
              </div>
            );
          })}
          ...
          <div>
            ...
            <div style={{ display:"flex", gap:4 }}>
              {[1,2,3,4,5].map(function(n) {
                return <button key={n} onClick={function() { upd("rating",n); }} style={{ background:"none", border:"none", cursor:"pointer", fontSize:22, color:(session.rating||0)>=n?C.yellow:C.textMuted, padding:"2px" }}>{"★"}</button>;
              })}
            </div>
          </div>
        </div>
      ...
      {venueModal ? <VenueModal locationName={venueModal} onClose={function() { setVM(null); }}/> : null}
    </Modal>
  );
}
        ...
        <div>
          ...
          <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
            {["Partner Meeting","Conference","Meal/Reception","Keynote","Logistics","Open Slot"].map(function(t) {
              return <button key={t} onClick={function(){sf("track",t);}} style={{ padding:"6px 10px", borderRadius:20, cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"'DM Sans',sans-serif", background:d.track===t?(TRACK_COLORS[t]||C.accent):C.bgDark, color:d.track===t?"#fff":C.textMuted, border:"1px solid "+(d.track===t?(TRACK_COLORS[t]||C.accent):C.border) }}>{t}</button>;
            })}
          </div>
        </div>
        <div>
          ...
          <div style={{ display:"flex", gap:6 }}>
            {DAYS.map(function(day) {
              return <button key={day.key} onClick={function(){sf("day",day.key);}} style={{ flex:1, padding:"7px 4px", borderRadius:10, cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"'DM Sans',sans-serif", background:d.day===day.key?C.accent:C.bgDark, color:d.day===day.key?"#fff":C.textMuted, border:"1px solid "+(d.day===day.key?C.accent:C.border) }}>{day.label}</button>;
            })}
          </div>
        </div>
  ...
  return (
    <Modal onClose={props.onClose} title={partner.name} wide>
      <div style={{ display:"flex", flexDirection:"column", gap:13 }}>
        ...
        <div>
          ...
          <div style={{ display:"flex", gap:6 }}>
            {["green","yellow","red"].map(function(s) {
              return <button key={s} onClick={function(){props.onUpdate(partner.id,"status",s);}} style={{ flex:1, padding:"7px", borderRadius:20, cursor:"pointer", fontSize:11, fontWeight:600, fontFamily:"'DM Sans',sans-serif", background:partner.status===s?STATUS_COLORS[s]:C.bgDark, color:partner.status===s?"#fff":C.textMuted, border:"1px solid "+(partner.status===s?STATUS_COLORS[s]:C.border) }}>{STATUS_LABELS[s]}</button>;
            })}
          </div>
        </div>
        ...
        {partner.pastNotes ? <div><FL>Previous Conference Notes</FL><div style={{ fontSize:12, color:C.textSub, lineHeight:1.6, background:C.bgDark, padding:"10px 12px", borderRadius:8, fontStyle:"italic" }}>{partner.pastNotes}</div></div> : null}
        {partner.attendees && partner.attendees.length > 0 ? <div><FL>Their Team</FL><AttendeeList attendees={partner.attendees}/></div> : null}
...
function EditPartnerModal(props) {
  ...
  return (
    <Modal onClose={props.onClose} title={"Edit - "+d.name} wide>
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {[{l:"Company Name",k:"name"},{l:"YTD Revenue",k:"revenue"},{l:"Meeting Objective",k:"objective"},{l:"Previous Conference Notes",k:"pastNotes"}].map(function(f) {
          return <div key={f.k}><FL>{f.l}</FL><input value={d[f.k]||""} onChange={function(e){sf(f.k,e.target.value);}} style={inp}/></div>;
        })}
          ...
          {d.openItems.map(function(oi,i) {
            return (
              <div key={oi.id} style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}>
                <input value={oi.text} onChange={function(e){setD(function(p){return Object.assign({},p,{openItems:p.openItems.map(function(x,j){return j===i?Object.assign({},x,{text:e.target.value}):x;})});});}} style={{...inp,flex:1}}/>
                <button onClick={function(){setD(function(p){return Object.assign({},p,{openItems:p.openItems.filter(function(_,j){return j!==i;})});});}} style={{background:"rgba(248,113,113,0.1)",color:C.red,border:"none",borderRadius:6,padding:"6px 10px",cursor:"pointer",fontSize:13}}>x</button>
              </div>
            );
          })}
          <button onClick={function(){setD(function(p){return Object.assign({},p,{openItems:p.openItems.concat([{id:"oi"+Date.now(),text:"",done:false}])});});}} style={{background:C.bgDark,color:C.textMuted,border:"1px dashed "+C.border,borderRadius:20,padding:"7px 14px",cursor:"pointer",fontSize:12,fontFamily:"'DM Sans',sans-serif",width:"100%",marginTop:4}}>+ Add Item</button>
        </div>
...
function QuickNotesModal(props) {
  return (
    <Modal onClose={props.onClose} title="Quick Notes">
      <textarea value={props.notes} onChange={function(e){props.onChange(e.target.value);}} placeholder="Jot anything down..." style={Object.assign({},ta,{minHeight:160,fontSize:14})}/>
      <GreenBtn onClick={props.onClose} style={{width:"100%",padding:"11px",fontSize:13,borderRadius:24,marginTop:8}}>Save and Close</GreenBtn>
    </Modal>
  );
}
...
function HotelModal(props) {
  ...
  return (
    <Modal onClose={props.onClose} title="Hotel Info">
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        {[{l:"Hotel Name",k:"name"},{l:"Phone",k:"phone"},{l:"Check-in",k:"checkIn"},{l:"Check-out",k:"checkOut"},{l:"My Room Number",k:"roomNumber"}].map(function(x){return <div key={x.k}><FL>{x.l}</FL><input value={d[x.k]||""} onChange={function(e){sf(x.k,e.target.value);}} style={inp}/></div>;})}
        <div><FL>Address</FL><AddressSearch value={d.address||""} onChange={function(v){sf("address",v);}} placeholder="Search hotel..."/></div>
        <GreenBtn onClick={function(){props.onChange(d);props.onClose();}} style={{width:"100%",padding:"11px",fontSize:13,borderRadius:24}}>Save</GreenBtn>
      </div>
    </Modal>
  );
}
  ...
  return (
    <Modal onClose={props.onClose} title="Post-Conference Export" wide>
      <div style={{ display:"flex", flexDirection:"column", gap:12 }}>
        ...
        <GreenBtn onClick={function(){try{navigator.clipboard.writeText(summary);}catch(e){}}} style={{width:"100%",padding:"11px",fontSize:13,borderRadius:24}}>Copy to Clipboard</GreenBtn>
      </div>
    </Modal>
  );
}
...
function PipChatModal(props) {
  ...
  function send() {
    ...
    fetch("https://api.anthropic.com/v1/messages",{
      method:"POST",
      headers:{"Content-Type":"application/json"},
      body:JSON.stringify({model:CLAUDE_MODEL,max_tokens:600,system:getRoleCtx(role,customTitle)+"\n\nContext: "+ctx,messages:history}),
    }).then(function(r){return r.json();})
    .then(function(data){
      var t = (data.content&&data.content[0]&&data.content[0].text)||"I couldn't respond right now.";
      setMsgs(function(p){return p.concat([{sender:"assistant",content:t}]);});
    }).catch(function(){
          ...
          {msgs.map(function(m,i) {
            return (
              <div key={i} style={{ display:"flex", justifyContent:m.sender==="user"?"flex-end":"flex-start" }}>
                <div style={{ maxWidth:"85%", background:m.sender==="user"?C.accent:C.bgCardAlt, color:m.sender==="user"?"#fff":C.text, padding:"10px 14px", borderRadius:m.sender==="user"?"18px 18px 4px 18px":"18px 18px 18px 4px", fontSize:13, lineHeight:1.6 }}>
                  {m.content}
                </div>
              </div>
            );
    ...
    <Modal onClose={props.onClose} title="">
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <div style={{ textAlign:"center", paddingBottom:14, borderBottom:"1px solid "+C.border }}>
          <div style={{ display:"flex", justifyContent:"center", marginBottom:10 }}><PipMark size={20} color={C.accent} glow pulse={loading}/></div>
        ...
        {loading ? <div style={{ textAlign:"center", padding:"20px", color:C.textMuted, fontSize:13 }}>Pip is preparing your briefing...</div> : <div style={{ fontSize:13, color:C.textSub, lineHeight:1.8, whiteSpace:"pre-wrap" }}>{text}</div>}
        <GreenBtn onClick={props.onClose} style={{ width:"100%", padding:"11px", fontSize:13, borderRadius:24 }}>{"Let's go"}</GreenBtn>
      </div>
    </Modal>
  );
}
  ...
  return (
    <Modal onClose={props.onClose} title="" wide>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        {loading ? <div style={{ textAlign:"center", padding:"20px", color:C.textMuted }}>Pip is analyzing your conference...</div> : <div style={{ fontSize:13, color:C.textSub, lineHeight:1.8, whiteSpace:"pre-wrap" }}>{text}</div>}
...
function RelHealthModal(props) {
  ...
  useEffect(function() {
    ...
    var ctx = props.partners.map(function(p) {
      ...
      return p.name+": Revenue "+p.revenue+", Status "+STATUS_LABELS[p.status]+", Tier "+p.tier+(open>0?", "+open+" unresolved items":"")+(p.pastNotes?", Notes: "+p.pastNotes.slice(0,80):"");
    }).join("\n");
    askPip("Analyze these partner relationships. Which need the most attention? What should I prioritize after this conference?", props.role, props.customTitle, ctx, function(err, result) {
      setText(result);
      setLoading(false);
    });
  }, []);
  return (
    <Modal onClose={props.onClose} title="" wide>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        {loading ? <div style={{ textAlign:"center", padding:"20px", color:C.textMuted }}>Analyzing...</div> : <div style={{ fontSize:13, color:C.textSub, lineHeight:1.8, whiteSpace:"pre-wrap" }}>{text}</div>}
        <GreenBtn onClick={props.onClose} style={{ width:"100%", padding:"11px", fontSize:13, borderRadius:24 }}>Done</GreenBtn>
      </div>
    </Modal>
  );
  ...
  if (!enterMode) return (
    <Modal onClose={props.onClose} title="Share with Team">
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ fontSize:13, color:C.textSub, textAlign:"center", lineHeight:1.7 }}>Generate a code to share your full schedule and partner data with your team.</div>
        {!generated ? (
          <GreenBtn onClick={generate} disabled={loading} style={{ width:"100%", padding:"12px", fontSize:14, borderRadius:24 }}>{loading?"Generating...":"Generate Share Code"}</GreenBtn>
        ) : (
          <div style={{ textAlign:"center" }}>
            ...
            <button onClick={function(){try{navigator.clipboard.writeText(code);}catch(e){}}} style={Object.assign({},btnBase,{background:C.bgCardAlt,color:C.textSub,border:"1px solid "+C.border})}>Copy Code</button>
          </div>
        )}
        {err ? <div style={{ fontSize:12, color:C.red, textAlign:"center" }}>{err}</div> : null}
        <button onClick={function(){setEnterMode(true);}} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif", textAlign:"center" }}>I have a code - Enter it here</button>
      </div>
    </Modal>
  );
  return (
    <Modal onClose={function(){setEnterMode(false);}} title="Enter Share Code">
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        <input value={inputCode} onChange={function(e){setInputCode(e.target.value.toUpperCase());}} placeholder="Enter code e.g. ABC123" style={Object.assign({},inp,{textAlign:"center",fontSize:18,fontWeight:700,letterSpacing:"0.2em",fontFamily:"monospace"})}/>
        {err ? <div style={{ fontSize:12, color:C.red, textAlign:"center" }}>{err}</div> : null}
        <GreenBtn onClick={loadCode} disabled={loadingCode||!inputCode.trim()} style={{ width:"100%", padding:"12px", fontSize:14, borderRadius:24 }}>{loadingCode?"Loading...":"Load Schedule"}</GreenBtn>
        <button onClick={function(){setEnterMode(false);}} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif", textAlign:"center" }}>Back</button>
      </div>
    </Modal>
  );
}
...
function AIImportModal(props) {
  ...
  function runImport() {
    ...
    .then(function(data){
      ...
      if (Array.isArray(parsed) && parsed.length > 0) { props.onImport(parsed); }
      else { setErrMsg("Couldn't find events. Try pasting more."); }
    }).catch(function(){setErrMsg("Something went wrong. Try plain text.");})
  ...
  if (!mode) return (
    <Modal onClose={props.onClose} title="Import Your Schedule">
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:10 }}>
        {[{id:"paste",icon:"paste",label:"Paste your schedule",desc:"Copy from email, Word, or anywhere"},{id:"url",icon:"link",label:"Drop in a URL",desc:"Paste a link to your conference agenda"},{id:"file",icon:"file",label:"Upload a file",desc:"PDF, Word doc, or spreadsheet"}].map(function(opt) {
          return (
            <button key={opt.id} onClick={function(){setMode(opt.id);}} style={{ background:C.bgCardAlt, border:"1px solid "+C.border, borderRadius:12, padding:"14px 16px", cursor:"pointer", display:"flex", alignItems:"center", gap:14, textAlign:"left" }}>
        ...
        <button onClick={props.onClose} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif", padding:"8px" }}>{"I'll build it manually"}</button>
      </div>
    </Modal>
  );
  return (
    <Modal onClose={function(){setMode(null);}} title={mode==="url"?"Conference URL":"Paste Schedule"}>
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        {mode==="url" ? <input value={inputText} onChange={function(e){setInputText(e.target.value);}} placeholder="https://conference.com/agenda" style={inp}/> : <textarea value={inputText} onChange={function(e){setInputText(e.target.value);}} placeholder="Paste your schedule here..." style={Object.assign({},ta,{minHeight:180})}/>}
        {errMsg ? <div style={{ fontSize:12, color:C.red, background:"rgba(248,113,113,0.1)", border:"1px solid rgba(248,113,113,0.2)", borderRadius:8, padding:"10px 12px" }}>{errMsg}</div> : null}
        <GreenBtn onClick={runImport} style={{ width:"100%", padding:"12px", fontSize:14, borderRadius:24 }} disabled={isLoading}>{isLoading?"Pip is reading...":"Let Pip Read It"}</GreenBtn>
        <button onClick={function(){setMode(null);}} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif" }}>Back</button>
      </div>
    </Modal>
  );
}
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:14 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:6, maxHeight:360, overflowY:"auto" }}>
          {importedSessions.map(function(s,i) {
            ...
            return (
              <div key={i} onClick={function(){toggle(i);}} style={{ background:isSelected?"rgba("+toRgb(color)+",0.1)":C.bgDark, border:"1px solid "+(isSelected?color+"40":C.border), borderLeft:"3px solid "+(isSelected?color:"transparent"), borderRadius:10, padding:"11px 14px", cursor:"pointer", display:"flex", gap:12, alignItems:"center" }}>
      ...
      <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
        ...
        <div style={{ background:C.bgDark, border:"1px solid "+C.border, borderRadius:10, padding:"12px 14px" }}>
          <div style={{ fontSize:13, color:C.textSub, lineHeight:1.7 }}>
            ...
            {"."}
            {upcoming.length > 0 ? <span>{" Next up: "}<span style={{ color:C.text, fontWeight:500 }}>{upcoming[0].title}</span>{" at "}<span style={{ color:C.accent }}>{upcoming[0].time}</span>{"."}</span> : null}
          </div>
        </div>
        <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
          {todaySess.map(function(s) {
            ...
            return (
              <div key={s.id} onClick={function(){props.onSelectS(s);props.onClose();}} style={{ background:C.bgDark, border:"1px solid "+C.border, borderLeft:"3px solid "+color, borderRadius:10, padding:"11px 14px", cursor:"pointer" }}>
                <div style={{ fontSize:13, fontWeight:500, color:C.text, marginBottom:4 }}>{s.title}</div>
                <div style={{ fontSize:11, color:C.textMuted }}>{s.time+(s.end?" - "+s.end:"")+(s.location?" | "+s.location:"")}</div>
                {p ? <div style={{ fontSize:11, color:C.textSub, marginTop:3 }}>{p.revenue+" | "+(p.attendees.filter(function(a){return a.poc;}).map(function(a){return a.name;}).join(", ")||"No POC set")}</div> : null}
                {openCount > 0 ? <div style={{ fontSize:10, color:C.accentOrange, marginTop:3 }}>{openCount+" open item"+(openCount>1?"s":"")}</div> : null}
              </div>
            );
          })}
          {todaySess.length === 0 ? <div style={{ textAlign:"center", padding:"20px", color:C.textMuted, fontSize:13 }}>No events scheduled for today</div> : null}
        </div>
      </div>
    </Modal>
  );
...
// Reads URL params like ?invite=kyle&role=vp&name=Kyle%20McChesney
...
function InviteWelcome(props) {
  ...
  var [pipText, setPipText] = useState("");
  ...
  var stepContent = [
    ...
    {
      title: "Hey " + name,
      body: "Chris set this up for you. I'm Pip - your AI conference assistant for ABPA 2026 in Indian Wells.",
      pipPrompt: "Give a warm 2-sentence welcome to " + name + " who is a " + roleDesc + ". They are about to use a conference app called Lanyard that Chris Vasconcellos built for the ABPA 2026 conference. Be personal and excited.",
    },
    // Step 1 - Schedule
    {
      title: "Your Schedule",
      body: "The full ABPA conference is already loaded - every session, every partner meeting, every dinner. May 18-21, Indian Wells.",
      pipPrompt: "In 2 sentences, tell " + name + " (a " + roleDesc + ") what to expect from their conference schedule at ABPA 2026. Mention there are partner meetings, keynote sessions, and networking events all pre-loaded.",
    },
    // Step 2 - Partners
    {
      title: "Key Partners",
      body: "All 9 partner profiles are ready - revenue, open items, who you're meeting, and what matters going in.",
      pipPrompt: "In 2 sentences, explain to " + name + " (a " + roleDesc + ") why having partner context before walking into meetings matters. Mention that LKQ at $222M is the top account and has some open items that need attention.",
    },
    // Step 3 - Meeting prep
    {
      title: "I've Got You Covered",
      body: "Tap any meeting and ask me for a brief, talking points, or a risk flag. I know your role and I'll give you exactly what you need.",
      pipPrompt: "In 2 sentences, tell " + name + " (a " + roleDesc + ") how you as Pip will help them in meetings. Be specific about the role - " + (role === "vp" ? "short executive summaries" : role === "dir" ? "strategic context" : "full operational detail") + ".",
    },
    // Step 4 - Go
    {
      title: "You're All Set",
      body: "Everything Chris built is here for you. Let's make this conference count.",
      pipPrompt: null,
    },
  ];

  var current = stepContent[step];

  useEffect(function() {
    if (current.pipPrompt && !pipText) {
      setLoading(true);
      askPip(current.pipPrompt, role, "", "", function(err, result) {
        setPipText(result);
        setLoading(false);
      });
    }
          ...
          {step === STEPS.length - 1 && (
            <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:24 }}>
              {[
                ...
                { l:"Sessions Loaded", v:"26", c:C.blue },
        ...
        <div>
          <div style={{ display:"flex", gap:5, justifyContent:"center", marginBottom:24 }}>
            {STEPS.map(function(s, i) {
              return <div key={i} style={{ width:i === step ? 24 : 6, height:6, borderRadius:3, background:i === step ? C.accent : i < step ? C.accentDim : C.bgPillActive, transition:"all 0.2s" }}/>;
            })}
          </div>
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:32, color:C.text }}>
      ...
      <div style={{ maxWidth:360, width:"100%", textAlign:"center" }}>
        <div style={{ display:"flex", justifyContent:"center", marginBottom:20 }}><PipMark size={24} color={C.accent} glow pulse/></div>
        <div style={{ fontSize:22, fontWeight:600, color:C.text, marginBottom:8 }}>One quick thing</div>
        <div style={{ fontSize:13, color:C.textSub, marginBottom:28, lineHeight:1.7 }}>{"What's your role? Pip will tailor everything to you."}</div>
        <div style={{ display:"flex", flexDirection:"column", gap:10, marginBottom:24 }}>
          {ROLES.map(function(r) {
            return (
              <button key={r.id} onClick={function(){setSelected(r.id);}} style={{ background:selected===r.id?C.bgPillActive:C.bgCard, border:"1px solid "+(selected===r.id?C.accent:C.border), borderRadius:12, padding:"14px 16px", cursor:"pointer", textAlign:"left" }}>
        ...
        {selected==="custom" ? <input value={customTitle} onChange={function(e){setCustomTitle(e.target.value);}} placeholder="Enter your title..." style={Object.assign({},inp,{marginBottom:16,textAlign:"center"})}/> : null}
        <GreenBtn onClick={function(){props.onSave(selected, customTitle);}} style={{ width:"100%", padding:"13px", fontSize:15, borderRadius:24 }}>{"Set Up Pip"}</GreenBtn>
      </div>
    </div>
  );
}
  ...
  function renderStep() {
    ...
    if (step === 1) return (
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", flex:1, textAlign:"center", padding:"0 32px" }}>
        <div style={{ marginBottom:28, position:"relative" }}>
          ...
          <div style={{ position:"relative", zIndex:1, display:"flex", justifyContent:"center" }}><PipMark size={32} color={C.accent} glow pulse/></div>
        </div>
        <div style={{ fontSize:24, fontWeight:600, color:C.text, marginBottom:12 }}>Meet Pip</div>
        <div style={{ fontSize:14, color:C.textSub, lineHeight:1.8, maxWidth:280 }}>{"Hi, I'm Pip - your AI conference assistant. I'll help you prep for meetings, take notes, and keep you one step ahead all conference long."}</div>
      </div>
    );
    if (step === 2) return (
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", flex:1, textAlign:"center", padding:"0 32px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:10, width:"100%", maxWidth:300 }}>
          {[{icon:"📋",text:"Full conference schedule in one place"},{icon:"🤝",text:"Partner profiles with revenue and open items"},{icon:"📝",text:"Meeting notes, action items, follow ups"},{icon:"✨",text:"Powered by Pip"}].map(function(item) {
            return (
    ...
    return (
      <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", flex:1, textAlign:"center", padding:"0 32px" }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:9, width:"100%", maxWidth:300, marginBottom:16 }}>
          {["Paste your schedule","Drop in a URL","Upload a file"].map(function(label) {
            return (
              <div key={label} style={{ background:C.bgCard, border:"1px solid "+C.border, borderRadius:10, padding:"11px 14px", display:"flex", alignItems:"center", gap:12 }}>
        ...
        <GreenBtn onClick={function(){setShowImport(true);}} style={{ padding:"12px 32px", fontSize:14, marginBottom:12 }}>Import with Pip</GreenBtn>
        <button onClick={props.onDone} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif" }}>{"Skip - add manually"}</button>
      </div>
    );
  }
  return (
      ...
      <div style={{ flex:1, display:"flex", flexDirection:"column", maxWidth:400, margin:"0 auto", width:"100%", paddingTop:60, paddingBottom:40 }}>
        ...
        <div style={{ padding:"0 32px" }}>
          <div style={{ display:"flex", gap:6, justifyContent:"center", marginBottom:24 }}>
            {Array.from({length:totalSteps}).map(function(_,i){return <div key={i} style={{ width:i===step?24:6, height:6, borderRadius:3, background:i===step?C.accent:C.bgPillActive, transition:"all 0.2s" }}/>;})}
          </div>
          {step < totalSteps-1 && step !== 3 ? <GreenBtn onClick={function(){setStep(function(s){return s+1;});}} style={{ width:"100%", padding:"13px", fontSize:15, borderRadius:24 }}>{"Continue"}</GreenBtn> : null}
          {step > 0 ? <button onClick={function(){setStep(function(s){return s-1;});}} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:13, fontFamily:"'DM Sans',sans-serif", display:"block", width:"100%", marginTop:12, textAlign:"center" }}>Back</button> : null}
          {step === 0 ? <button onClick={props.onDone} style={{ background:"none", border:"none", color:C.textMuted, cursor:"pointer", fontSize:12, fontFamily:"'DM Sans',sans-serif", display:"block", width:"100%", marginTop:12, textAlign:"center" }}>Skip intro</button> : null}
        </div>
      </div>
  ...
  return (
    <div>
        ...
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          ...
          {weather ? (
            <div style={{ textAlign:"right" }}>
              ...
              <div style={{ fontSize:10, color:C.textMuted }}>{getWeatherIcon(weather.code)}</div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:10 }}>
        {[{l:"Meetings",v:sessions.filter(function(s){return s.track==="Partner Meeting";}).length,c:C.accentOrange},{l:"Partners",v:partners.length,c:C.accent},{l:"Pending",v:partners.filter(function(p){return p.unscheduled;}).length,c:C.yellow},{l:"At Risk",v:partners.filter(function(p){return p.status==="red";}).length,c:C.red}].map(function(s) {
          return <Card key={s.l} style={{ padding:"13px 15px" }}><div style={{ fontSize:22, fontWeight:600, color:s.c }}>{s.v}</div><div style={{ fontSize:10, color:C.textMuted, marginTop:3 }}>{s.l}</div></Card>;
        })}
      </div>
      ...
      <Card style={{ padding:"12px 16px", marginBottom:10, cursor:"pointer", background:props.quickNote?C.bgCardAlt:C.bgCard }} onClick={props.onQuickNote}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:11, color:C.textMuted, marginTop:1 }}>{props.quickNote?props.quickNote.slice(0,40)+(props.quickNote.length>40?"...":""):"Tap to jot something down"}</div>
          </div>
          <div style={{ fontSize:13, color:C.textMuted }}>{">"}</div>
        </div>
      </Card>
      <Card style={{ padding:"13px 15px", marginBottom:10 }}>
        <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start" }}>
          <div style={{ flex:1 }}>
            ...
            <div style={{ fontSize:11, color:C.textSub, lineHeight:1.6 }}>{hotel.address}<br/>{hotel.phone}<br/>{"Check-in: "+hotel.checkIn}{hotel.roomNumber?<span><br/>{"Room: "+hotel.roomNumber}</span>:null}</div>
          </div>
              ...
              <div key={s.id}>
                <Card accent={hasC?C.red:isOpen?undefined:color} style={{ padding:"11px 14px", display:"flex", gap:12, alignItems:"flex-start", cursor:isOpen?"default":"pointer", opacity:isOpen?0.7:1 }} onClick={function(){if(!isOpen&&!s.isParent)props.onSelectS(s);if(hasKids)setExp(isExp?null:s.id);}}>
                  <div style={{ minWidth:60, textAlign:"right", paddingTop:2, flexShrink:0 }}>
                    {s.time ? <div><div style={{ fontSize:11, fontWeight:500, color:C.textSub }}>{s.time}</div>{s.end?<div style={{ fontSize:9, color:C.textMuted, marginTop:1 }}>{"-> "+s.end}</div>:null}</div> : <div style={{ fontSize:10, color:C.textMuted }}>{"--"}</div>}
                  </div>
                  <div style={{ flex:1, minWidth:0 }}>
                    ...
                    {s.location ? <div onClick={function(e){e.stopPropagation();if(vKey)props.onSelectV(s.location);}} style={{ fontSize:10, color:vKey?C.accent:C.textMuted, marginTop:3, cursor:vKey?"pointer":"default" }}>{s.location+(vKey?" >":"")}</div> : null}
                    {s.notes && !isOpen ? <div style={{ fontSize:10, color:C.textMuted, marginTop:3, lineHeight:1.5 }}>{s.notes}</div> : null}
                    {!isOpen ? <div style={{ display:"flex", gap:4, marginTop:5, flexWrap:"wrap" }}>{TEAM.filter(function(t){return s.attendees&&s.attendees.includes(t.key);}).map(function(t){return <span key={t.key} style={{ background:"rgba("+toRgb(t.color)+",0.12)", color:t.color, fontSize:9, fontWeight:600, padding:"2px 7px", borderRadius:20 }}>{t.name}</span>;})}</div> : null}
                  </div>
                    ...
                    {kids.map(function(child) {
                      return (
                        <div key={child.id} style={{ background:C.bgDark, border:"1px solid "+C.border, borderLeft:"2px solid "+C.yellow, borderRadius:8, padding:"8px 12px", display:"flex", gap:10 }}>
                          <div style={{ minWidth:54, textAlign:"right", flexShrink:0 }}><div style={{ fontSize:10, color:C.textSub }}>{child.time}</div><div style={{ fontSize:9, color:C.textMuted }}>{"-> "+child.end}</div></div>
...
function PartnersView(props) {
  ...
  return (
    <div>
      ...
      {partners.length === 0 ? <EmptyState title="No partners yet" body="Add partner profiles to track relationships." action="Add Your First Partner" onAction={props.onAdd}/> : (
                ...
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:8 }}>
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ display:"flex", gap:4, marginTop:6, flexWrap:"wrap" }}>
                      {p.attendees && p.attendees.filter(function(a){return a.poc;}).map(function(a){return <span key={a.id} style={{ background:"rgba("+toRgb(C.accent)+",0.1)", color:C.accent, fontSize:9, fontWeight:600, padding:"2px 7px", borderRadius:20 }}>{"POC: "+a.name}</span>;})}
                    </div>
                  </div>
                </div>
  ...
  return (
    <div>
      <Card style={{ padding:"13px 16px", marginBottom:16, background:C.bgCardAlt }}>
        ...
        {openSlots.length === 0 ? <div style={{ fontSize:12, color:C.textMuted, fontStyle:"italic" }}>No open slots</div> : openSlots.map(function(s){var d=DAYS.find(function(x){return x.key===s.day;})||{label:"",date:""};return <div key={s.id} style={{ display:"flex", gap:10, marginBottom:4, fontSize:12 }}><span style={{ color:C.purple, fontWeight:500, minWidth:64 }}>{d.label+" "+d.date}</span><span style={{ color:C.textSub }}>{s.time+" - "+s.end}</span></div>;})}
      </Card>
      {unsched.length === 0 ? <EmptyState title="All partners scheduled!" body="Every partner has a meeting slot."/> : (
        <div style={{ display:"flex", flexDirection:"column", gap:9 }}>
              ...
              <Card key={p.id} accent={C.accentOrange} style={{ padding:"13px 16px" }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10, marginBottom:10 }}>
                  <div>
                    ...
                    {openSlots.map(function(s){var d=DAYS.find(function(x){return x.key===s.day;})||{label:""};return <div key={s.id} style={{ fontSize:10, color:C.purple, marginTop:2 }}>{d.label+" "+s.time+"-"+s.end}</div>;})}
                  </div>
                  <Pill color={C.accentOrange}>Pending</Pill>
                </div>
                <GreenBtn onClick={function(){props.onSchedule(p);}} style={{ width:"100%", fontSize:12 }}>Schedule Meeting</GreenBtn>
              </Card>
            );
          })}
        </div>
...
export default function Lanyard() {
  ...
  useEffect(function() {
    ...
    setSess(function(prev) {
      return prev.map(function(s) {
        ...
        var start = parseTime(s.time), end = s.end ? parseTime(s.end) : start+60;
  ...
  useEffect(function() {
    if (alertMsg) { var t = setTimeout(function(){setAlertMsg(null);}, 6000); return function(){clearTimeout(t);}; }
  }, [alertMsg]);
  ...
  useEffect(function() {
    ...
    .then(function(r){return r.json();})
    .then(function(d){if(d&&d.current)setWeather({temp:Math.round(d.current.temperature_2m),code:d.current.weathercode});})
    .catch(function(){});
  }, []);

  // Auto-generate open slots whenever sessions change
  useEffect(function() {
    setSess(function(prev) {
      ...
      if (prevAuto !== newAuto) return withSlots;
      return prev;
    });
  ...
  function saveEditS(draft) {
    if (!draft) { setSess(function(p){return p.filter(function(s){return s.id!==editS.id;});}); setSelS(null); }
    else { setSess(function(p){return p.map(function(s){return s.id===draft.id?Object.assign({},draft):s;});}); setSelS(Object.assign({},draft)); }
    setEditS(null); notify(draft?"Event updated":"Event deleted");
  }
  function saveEditP(draft) {
    ...
    setEditP(null); notify("Partner updated");
  }
  function addEvent(f) {
    if (!f.title || !f.time) return;
    var s = Object.assign({}, f, { id:Date.now(), track:f.type, attendees:["C","K","T"], isParent:false, isChild:false, status:"upcoming", partnerId:f.partnerId||null });
    setSess(function(p){return p.concat([s]);});
    if (f.partnerId) updateP(f.partnerId, "unscheduled", false);
    setAddE(false); setSchedP(null); notify("Event added");
  }
  function addPartner(f) {
    if (!f.name) return;
    setPart(function(p){return p.concat([Object.assign({},f,{id:"p"+Date.now(),openItems:[],attendees:[],rating:0,pastNotes:"",unscheduled:true,scheduledMeeting:""})]);});
    setAddP(false); notify("Partner added");
  }
  function handleImport(importedSessions) {
    var mapped = importedSessions.map(function(s,i){
      return { id:"import_"+Date.now()+"_"+i, day:s.day&&s.day.toString().includes("1")?"mon":s.day&&s.day.toString().includes("2")?"tue":s.day&&s.day.toString().includes("3")?"wed":"thu", time:s.time||"9:00 AM", end:s.end||"", title:s.title||"Untitled", location:s.location||"", track:s.track||"Conference", attendees:["C","K","T"], notes:"", status:"upcoming", partnerId:null, isParent:false, isChild:false };
    });
  ...
  var conflicts = useMemo(function() {
    var ids = new Set(), g = {};
    sessions.forEach(function(s){if(!s.time||s.isChild)return;if(!g[s.day])g[s.day]=[];g[s.day].push(s);});
    Object.values(g).forEach(function(arr){for(var i=0;i<arr.length;i++)for(var j=i+1;j<arr.length;j++)if(arr[i].time===arr[j].time){ids.add(arr[i].id);ids.add(arr[j].id);}});
    return ids;
  }, [sessions]);

  var pMap = useMemo(function() { var m={}; partners.forEach(function(p){m[p.id]=p;}); return m; }, [partners]);
  var openSlots = sessions.filter(function(s){return s.track==="Open Slot";});
  var showDayTabs = view==="schedule"||view==="team";
  var srchRes = useMemo(function() {
    if (!srchQ.trim()) return { s:[], p:[] };
    var q = srchQ.toLowerCase();
    return { s:sessions.filter(function(x){return x.title.toLowerCase().includes(q)||(x.location||"").toLowerCase().includes(q);}), p:partners.filter(function(x){return x.name.toLowerCase().includes(q);}) };
  ...
  // Show invite welcome if coming from an invite link and haven't seen it yet
  if (inviteParams && !inviteDone) {
    return <InviteWelcome
      ...
      onDone={function() {
        ...
        // Auto-set role from invite params
  ...
  return (
    <div style={{ fontFamily:"'DM Sans',sans-serif", background:C.bg, minHeight:"100vh", color:C.text }}>
      <link href="https://fonts.googleapis.com/css2?family=DM+Sans:wght@300;400;500;600;700&display=swap" rel="stylesheet"/>
      <Toast message={toast}/>
      {alertMsg ? <div style={{ position:"fixed", top:0, left:"50%", transform:"translateX(-50%)", width:"100%", maxWidth:480, zIndex:9998, background:C.accentOrange, color:"#fff", padding:"12px 20px", textAlign:"center", fontSize:13, fontWeight:600, fontFamily:"'DM Sans',sans-serif" }}>{alertMsg}</div> : null}
        ...
        <div style={{ maxWidth:480, margin:"0 auto" }}>
            ...
            <div style={{ paddingBottom:8 }}>
              ...
              {srchQ ? (
                <div style={{ background:C.bgDark, border:"1px solid "+C.border, borderRadius:10, marginTop:4 }}>
                  {srchRes.s.map(function(s){return <div key={s.id} onClick={function(){setSelS(s);setSearch(false);setSrchQ("");}} style={{ padding:"9px 14px", borderBottom:"1px solid "+C.border, cursor:"pointer", fontSize:12, color:C.text }}>{s.title+" | "+(DAYS.find(function(d){return d.key===s.day;})||{label:""}).label+" "+s.time}</div>;})}
                  {srchRes.p.map(function(p){return <div key={p.id} onClick={function(){setSelP(p);setSearch(false);setSrchQ("");}} style={{ padding:"9px 14px", borderBottom:"1px solid "+C.border, cursor:"pointer", fontSize:12, color:C.text }}>{p.name+" - "+p.revenue}</div>;})}
          ...
          <div style={{ display:"flex", background:"rgba(0,0,0,0.2)", borderRadius:10, padding:3, gap:2 }}>
            {[["home","Home"],["schedule","Schedule"],["team","Team"],["partners","Partners"],["pending","Pending"]].map(function(pair) {
              return <button key={pair[0]} onClick={function(){setView(pair[0]);}} style={{ flex:1, padding:"6px 4px", borderRadius:8, cursor:"pointer", fontSize:9, fontWeight:600, fontFamily:"'DM Sans',sans-serif", background:view===pair[0]?C.bgPillActive:"transparent", color:view===pair[0]?C.accent:C.textMuted, border:"1px solid "+(view===pair[0]?C.border:"transparent") }}>{pair[1]}</button>;
            })}
          </div>
        </div>
      </div>
      ...
      {showDayTabs ? (
        <div style={{ padding:"12px 18px 0", maxWidth:480, margin:"0 auto" }}>
          <div style={{ display:"flex", gap:5 }}>
            {DAYS.map(function(d) {
              return <button key={d.key} onClick={function(){setDay(d.key);}} style={{ flex:1, padding:"8px 4px", borderRadius:10, cursor:"pointer", fontFamily:"'DM Sans',sans-serif", border:"none", background:day===d.key?C.bgPillActive:C.bgPill }}>
      ...
      <div style={{ maxWidth:480, margin:"0 auto", padding:"0 18px 28px" }}>
        <div style={{ borderTop:"1px solid "+C.border, paddingTop:12, display:"flex", gap:12, flexWrap:"wrap" }}>
          {Object.entries(TRACK_COLORS).map(function(entry) {
            return <div key={entry[0]} style={{ display:"flex", alignItems:"center", gap:4 }}><div style={{ width:7, height:7, borderRadius:2, background:entry[1] }}/><span style={{ fontSize:10, color:C.textMuted }}>{entry[0]}</span></div>;
          })}
        </div>
      </div>
      ...
      {selS && !editS ? <MeetingModal session={selS} pMap={pMap} hasConflict={conflicts.has(selS.id)} onClose={function(){setSelS(null);}} onUpdateS={updateS} onUpdateP={updateP} onEdit={function(s){setEditS(Object.assign({},s));}} role={role} customTitle={customTitle}/> : null}
      {editS ? <EditSessionModal session={editS} onClose={function(){setEditS(null);}} onSave={saveEditS}/> : null}
 ...
 : null}
      {editP ? <EditPartnerModal partner={editP} onClose={function(){setEditP(null);}} onSave={saveEditP}/> : null}
      {schedP ? <ScheduleMtgModal partner={schedP} openSlots={openSlots} onClose={function(){setSchedP(null);}} onAdd={addEvent}/> : null}
      {showAddE ? <AddEventModal partners={partners} openSlots={openSlots} onClose={function(){setAddE(false);}} onAdd={addEvent}/> : null}
      {showAddP ? <AddPartnerModal onClose={function(){setAddP(false);}} onAdd={addPartner}/> : null}
      {venueModal ? <VenueModal locationName={venueModal} onClose={function(){setVM(null);}} /> : null}
      {showHotel ? <HotelModal hotel={hotel} onChange={function(d){setHotel(d);}} onClose={function(){setHotelM(false);}} /> : null}
 ...
 : null}
      {showImport ? <AIImportModal onClose={function(){setImport(false);}} onImport={handleImport}/> : null}
 ...
 : null}
      {showPipChat ? <PipChatModal onClose={function(){setPipChat(false);}} sessions={sessions} partners={partners} role={role} customTitle={customTitle} now={now}/> : null}
      {showBriefing ? <DailyBriefingModal onClose={function(){setBriefing(false);}} sessions={sessions} partners={partners} role={role} customTitle={customTitle} now={now}/> : null}
      {showDebrief ? <DebriefModal onClose={function(){setDebrief(false);}} sessions={sessions} partners={partners} role={role} customTitle={customTitle}/> : null}
      {showRelHealth ? <RelHealthModal onClose={function(){setRelHealth(false);}} partners={partners} role={role} customTitle={customTitle}/> : null}
      {showShare ? <ShareModal onClose={function(){setShare(false);}} sessions={sessions} partners={partners} hotel={hotel}/> : null}
